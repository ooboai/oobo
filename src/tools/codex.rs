...
fn session_from_rollout(path: &Path) -> Option<Session> {
    ...
    for line in content.lines() {
        ...
        if event_type == "session_start" || event_type == "session_meta" {
            if let Some(cwd) = v
                .get("payload")
        ...
        if name.is_empty() && event_type == "event_msg" {
                ...
                .and_then(|t| t.as_str())
            {
                if msg_type == "user_message" {
                    ...
                    {
                        name =
                            crate::utils::truncate_name(text, crate::utils::MAX_SESSION_NAME_LEN);
                    }
                }
            }
        }

        if name.is_empty() && event_type == "response_item" {
                    ...
                    if let Some(content) = payload.get("content").and_then(|c| c.as_array()) {
                        for item in content {
                            if let Some(text) = item.get("text").and_then(|t| t.as_str()) {
                                let t = text.trim();
                                let is_instruction = t.starts_with('<')
                                    || t.starts_with('#')
                                    || t.starts_with("You are")
                                    || t.starts_with("Filesystem sandbox");
                                if !is_instruction && t.len() > 5 {
                                    name = crate::utils::truncate_name(
                                        t,
...
pub fn sessions_for_project(project_root: &str) -> Result<Vec<Session>, String> {
    ...
    let mut sessions = sessions_from_sqlite()?;
    if sessions.is_empty() {
        sessions = find_all_rollouts()
            .iter()
...
/// Fast session discovery from ~/.codex/state_5.sqlite threads table.
fn sessions_from_sqlite() -> Result<Vec<Session>, String> {
    let db_path = match codex_dir() {
        Some(d) => d.join("state_5.sqlite"),
    ...
    let conn = crate::utils::open_db_readonly(&db_path).map_err(|e| format!("codex db: {e}"))?;
    ...
    let mut stmt = conn
        ...
        .map_err(|e| format!("codex query: {e}"))?;
...
pub mod transcript {
        ...
        for entry in entries.flatten() {
            ...
            if p.is_dir() {
                ...
                if id == session_id
                    ...
                    || id.ends_with(session_id)
                    || id.contains(session_id)
                {
                    return Some(p);
                }
            }
    ...
    pub fn extract_stats(path: &Path) -> Option<crate::remote::payload::SessionStats> {
        ...
        Some(crate::remote::payload::SessionStats {
            ...
            is_estimated: false,
            token_source: if input_tokens > 0 {
    ...
    /// Try to detect the Codex model name from ~/.codex/models_cache.json.
    fn detect_model_from_cache() -> Option<String> {
        let path = super::codex_dir()?.join("models_cache.json");
        let content = fs::read_to_string(path).ok()?;
...
"#,
    ...
    fn test_parse_iso_timestamp() {
        let ts = crate::utils::parse_iso_timestamp("2025-06-01T10:00:00Z");
        assert!(ts.is_some());
        assert!(ts.unwrap() > 0);
    }
