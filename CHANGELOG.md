...
- **Privacy: gitignored files leaked in anchor metadata** -- `files_touched` in session metadata on the orphan branch previously included paths from `.docs/`, `.cursor/`, `tmp/`, and other gitignored directories. Now filtered via `git check-ignore` before writing anchors. Existing leaked paths scrubbed from the orphan branch history.
...
- **Legacy 0.1.x command hints** -- the hint system that intercepted old commands (`oobo scan`, `oobo alias`, `oobo sessions`, etc.) and printed migration messages has been removed. Unknown subcommands now produce clap's standard error.
