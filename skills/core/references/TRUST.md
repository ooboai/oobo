...
| Local-first | All data stays on the local orphan branch (`oobo/anchors/v2`; `oobo/anchors/v1` is read-only legacy, kept for backward compatibility). Anchor metadata is pushed only to your existing git remote (alongside your code) via the pre-push hook. No data goes to any external database from the CLI — everything is git-native. The optional remote API (recall, delta) requires a separate API key. |
...
Oobo captures AI context alongside commits, enriching them with anchor metadata (linked sessions, agent/human attribution, diff stats, token counts), and stores everything on the `oobo/anchors/v2` orphan branch (v1 is read-only legacy). Anchor data is git-native and fully contained within the repository.

## Agent Configuration

Agents should operate with **transparency on** - this is the recommended default for all automated workflows. Transparency mode writes anchor metadata and redacted transcripts to the local orphan branch (`oobo/anchors/v2`), giving teams visibility into AI contributions.
