...
2. **Project checklist:** list every discovered project with a checkbox. Checked = enabled (default). Space toggles, Enter continues.
3. **API key:** prompt to paste a key. Skippable. Stored via `settings set key`.
4. **Alias:** prompt `alias git=oobo? [y/N]`. On `y`, call the same code path as `oobo alias install`.
5. **Summary:** one screen showing what changed, exit.
...
oobo setup
  ...
  paste API key for oobo.ai cloud? (or press enter to skip)
  > sk_...

  alias `git` → `oobo`? [y/N]: y
...
**Side effects:**
- `projects` table rows inserted/updated for each discovered project.
- `project_settings.oobo = on|off` for each toggled project.
...
**Behavior:** Same semantics as `oobo alias uninstall`. Provided here so users who think of setup as the "manage my install" command can find it.
...
- Re-install `post-commit` / `post-merge` hooks if missing or outdated.
